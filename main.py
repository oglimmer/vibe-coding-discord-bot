...
class DiscordBot(commands.Bot):
    ...
    async def setup_hook(self):
        try:
            ...
            await setup_birthday_command(self)
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        try:
            ...
            if self.factcheck_handler:
                factcheck_handled = (
                    await self.factcheck_handler.handle_factcheck_reaction(
                        reaction, user
                    )
                )
            ...
            if greeting_id:
                ...
                if success:
                    logger.info(
                        f"Saved reaction {reaction.emoji} from {user.display_name} to greeting {greeting_id}"
                    )
                else:
                    logger.error(
                        f"Failed to save reaction {reaction.emoji} from {user.display_name} to greeting {greeting_id}"
                    )
    ...
    async def on_reaction_remove(self, reaction, user):
        ...
        try:
            ...
            if greeting_id:
                ...
                if success:
                    logger.info(
                        f"Removed reaction {reaction.emoji} from {user.display_name} to greeting {greeting_id}"
                    )
                else:
                    logger.error(
                        f"Failed to remove reaction {reaction.emoji} from {user.display_name} to greeting {greeting_id}"
                    )
