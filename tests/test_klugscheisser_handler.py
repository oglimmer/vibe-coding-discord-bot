...
class TestKlugscheisserHandler(unittest.TestCase):
    ...
    def test_should_klugscheiss_message_valid(self):
        ...
        with patch.object(
            self.handler.openai_service, "is_available", return_value=True
        ):
            # Mock db_manager get_klugscheisser_preference
            self.mock_db_manager.get_klugscheisser_preference.return_value = {
                "opted_in": True
            }
            # Run async test
            result = asyncio.run(
                self.handler.should_klugscheiss_message(self.mock_message)
            )
    ...
    def test_handle_klugscheisserei_success(self):
        ...
        async def async_test():
            ...
            with patch.object(
                self.handler.openai_service,
                "should_respond_with_klugscheiss",
                return_value=True,
            ):
                with patch.object(
                    self.handler.openai_service,
                    "generate_klugscheiss_response",
                    return_value="Test klugscheißer response",
                ):
                    # Mock message reply method
                    self.mock_message.reply = AsyncMock()

                    result = await self.handler.handle_klugscheisserei(
                        self.mock_message
                    )
