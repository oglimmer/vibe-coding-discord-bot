...
class TestBullshitBoard(unittest.TestCase):
    ...
    def test_bullshit_embed_formatting(self):
        ...
        embed = self.command._format_bullshit_embed(self.sample_board_data, 0, 1, 30)
        ...
        ranking_field = next(
            (field for field in embed.fields if field.name == "🏆 Ranking"), None
        )
        ...
        explanation_field = next(
            (field for field in embed.fields if "Score-Erklärung" in field.name), None
        )
    ...
    def test_empty_board_data(self):
        ...
        no_data_field = next(
            (field for field in embed.fields if "Keine Daten" in field.name), None
        )
    ...
    async def test_bullshit_command_execution(self):
        ...
        self.mock_db_manager.get_bullshit_board_data.return_value = (
            self.sample_board_data
        )
