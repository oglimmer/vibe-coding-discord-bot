...
class Game1337Logic:
    ...
    def get_daily_win_time(self, game_date: Optional[date] = None) -> datetime:
        ...
        if game_date in self._daily_win_times:
            logger.debug(
                f"Using cached win time for {game_date}: {self.format_time_with_ms(self._daily_win_times[game_date])}"
            )
    ...
    def parse_timestamp(
        self, timestamp_str: str, game_date: date
    ) -> Optional[datetime]:
        ...
        for i, pattern in enumerate(patterns):
            ...
            if match:
                ...
                if len(groups) == 4:  # hh:mm:ss.SSS
                    ...
                    game_start_time = self.parse_game_start_time()
                    hour, minute = (
                        str(game_start_time.hour),
                        str(game_start_time.minute),
                    )
                    second, ms = groups
                    ms = ms.ljust(3, "0")[:3]
                else:  # ss
                ...
                try:
                    ...
                    dt = datetime.combine(
                        game_date, time(hour, minute, second, microsecond)
                    )
                    logger.debug(
                        f"Successfully parsed timestamp: {self.format_time_with_ms(dt)}"
                    )
                    return dt
    ...
    def _select_winner_from_bets(
        ...
        logger.debug(
            f"Found {len(valid_bets)} valid bets (≤ win time) out of {len(daily_bets)} total"
        )
        ...
        identical_times = [
            bet for bet in valid_bets if bet["play_time"] == winner["play_time"]
        ]
    ...
    def determine_winner(
        ...
        """
        logger.info(
            f"Determining winner for {game_date}, win time: {self.format_time_with_ms(win_time)}"
        )
    ...
    def determine_and_save_winner(
        self, game_date: date, win_time: datetime
    ) -> Optional[Dict[str, Any]]:
        """Atomic read + select + persist. Use this from the scheduler path."""
        logger.info(
            f"Atomically deciding winner for {game_date}, win time: {self.format_time_with_ms(win_time)}"
        )
    ...
    def _apply_winner_selection_rules(
        self, valid_bets: List[Dict], win_time: datetime
    ) -> Optional[Dict]:
        """
        ...
        if regular_bets:
            closest_regular = min(
                regular_bets,
                key=lambda x: abs(
                    win_time_ms - self.get_milliseconds_since_midnight(x["play_time"])
                ),
            )

        closest_early_bird = None
        if early_bird_bets:
            closest_early_bird = min(
                early_bird_bets,
                key=lambda x: abs(
                    win_time_ms - self.get_milliseconds_since_midnight(x["play_time"])
                ),
        ...
        if not closest_regular:
            if closest_early_bird:
                logger.debug(
                    f"Winner: {closest_early_bird['username']} (early_bird bet, no regular bets)"
                )
        ...
        if not closest_early_bird:
            logger.debug(
                f"Winner: {closest_regular['username']} (regular bet, no early_bird bets)"
            )
            return closest_regular

        # Compare distances to win_time
        regular_distance = abs(
            win_time_ms
            - self.get_milliseconds_since_midnight(closest_regular["play_time"])
        )
        early_bird_distance = abs(
            win_time_ms
            - self.get_milliseconds_since_midnight(closest_early_bird["play_time"])
        )

        # If regular bet is closer, it wins
        if regular_distance <= early_bird_distance:
            logger.debug(
                f"Winner: {closest_regular['username']} (regular bet closer to win time)"
            )
            return closest_regular
        ...
        if time_diff > 3000:  # More than 3 seconds (3000ms)
            logger.debug(
                f"Winner: {closest_early_bird['username']} (early_bird closer and >3s apart)"
            )
            return closest_early_bird
        else:
            logger.debug(
                f"Winner: {closest_regular['username']} (early_bird closer but within 3s of regular)"
            )
            return closest_regular
    ...
    def validate_early_bird_timestamp(
        self, timestamp_str: str, current_time: Optional[datetime] = None
    ) -> Dict[str, Any]:
        """
        ...
        return {"valid": True, "timestamp": parsed_timestamp}
    ...
    def get_winner_stats(
        self, days: Optional[int] = None, user_id: Optional[int] = None
    ) -> List[Dict[str, Any]]:
    ...
    def create_user_info_embed_data(self, user_bet: Dict[str, Any]) -> Dict[str, Any]:
        ...
        embed_data = {"title": "🎯 Your 1337 Bet Info", "color": 0x1337FF, "fields": []}
    ...
    def create_winner_message(
        ...
        current_role_holders: Optional[Dict[str, Any]] = None,
    ) -> str:
        """Create plain text winner announcement message"""

        # Start with winner announcement
        bet_type = (
            "early bird" if winner_data["bet_type"] == "early_bird" else "regular"
        )
        message_lines = [
        ...
        if guild_id and current_role_holders:
            ...
            new_role_assignments = self.determine_new_role_assignments(
                winner_data, current_role_holders, guild_id
            )
            role_changes = []
                ...
                if current_user_id != new_user_id:
                    ...
                    if new_user_id:  # Only process if there's a new user being assigned
                        if role_type == "general":
                            user_to_display = next(
                                (
                                    p
                                    for p in top_365_players
                                    if p["user_id"] == new_user_id
                                ),
                                None,
                            )
                            ...
                            role_type == "sergeant"
                            and winner_data["user_id"] == new_user_id
                        ):
                        ...
                        if user_to_display:
                            ...
                            if user_to_display == winner_data:
                                role_changes.append(
                                    f"New {role_name}: {user_to_display['username']}"
                                )
                            else:
                                role_changes.append(
                                    f"New {role_name}: {user_to_display['username']} ({user_to_display['wins']} wins)"
                                )
            ...
            embed_data["fields"].append(
                {
                    ...
                    "value": self._format_stats_list(stats)
                    if stats
                    else "No winners yet",
                    "inline": False,
                }
                ...
                embed_data["fields"].append(
                    {
                        ...
                        "value": self._format_daily_bets(today_bets)
                        if today_bets
                        else "No bets today",
                        "inline": False,
                    }
                ...
                embed_data["fields"].append(
                    {
                        ...
                        "value": self._format_daily_bets(yesterday_bets)
                        if yesterday_bets
                        else "No bets yesterday",
                        "inline": False,
                    }
                )
