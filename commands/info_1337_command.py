...
class Info1337Command(commands.Cog):
    ...
    @discord.app_commands.command(
        name="1337-info", description="Show information about today's 1337 game"
    )
    ...
    def _create_pre_game_embed(self, user_bet):
        ...
        embed = discord.Embed(
            title="🎯 1337 Game Info", color=0x1337FF, timestamp=datetime.now()
        )
        ...
        if user_bet:
            ...
            embed.add_field(
                name="Status", value="✅ **Bet placed!** Good luck!", inline=False
            )
        else:
            # User has not placed a bet - prompt to bet
            embed.add_field(
                name="Status", value="❌ **No bet placed yet!**", inline=False
            )
        ...
        embed.set_footer(
            text=f"Game starts at {Config.GAME_START_TIME[:5]} - Place your bets!"
        )
        return embed
    ...
    def _create_post_game_embed(self, user_bet, game_date):
        ...
        if winner:
            ...
            embed.add_field(
                name="🏆 Winner", value=f"**{winner['username']}**", inline=True
            )
        ...
        embed.set_footer(
            text=f"Next game starts tomorrow at {Config.GAME_START_TIME[:5]}!"
        )
        return embed
