...
class Game1337Command(commands.Cog):
    ...
    async def _schedule_next_winner_determination(self):
        ...
        try:
            ...
            if self.winner_determination_task:
                logger.warning("Cancelling existing winner determination task")
                self.winner_determination_task.cancel()
    ...
    async def _delayed_winner_determination(self, delay_seconds):
        ...
        try:
            ...
            if time_diff > 0.1:  # More than 100ms off
                logger.warning(
                    f"Winner determination timing off by {time_diff:.3f} seconds"
                )

            logger.info(
                f"Starting winner determination at exact time: {now.strftime('%H:%M:%S.%f')}"
            )
    ...
    async def _determine_daily_winner(self):
        ...
        try:
            ...
            if now < win_time:
                ...
                logger.info(
                    f"Waiting {wait_seconds:.3f} seconds until win time: {self.game_logic.format_time_with_ms(win_time)}"
                )
                ...
                if wait_seconds > 0.01:  # More than 10ms to wait
                    await asyncio.sleep(
                        wait_seconds - 0.01
                    )  # Sleep until 10ms before target
                ...
                time_diff = (
                    actual_time - win_time
                ).total_seconds() * 1000  # Convert to milliseconds
                logger.info(
                    f"Winner determination triggered at: {self.game_logic.format_time_with_ms(actual_time)} (diff: {time_diff:.1f}ms)"
                )
            ...
            try:
                logger.debug(
                    f"Getting current role holders for {len(self.bot.guilds)} guilds"
                )
                for guild in self.bot.guilds:
                    try:
                        logger.debug(f"Processing guild: {guild.name} (ID: {guild.id})")
                        guild_roles = {
                            "general": self.db_manager.get_role_assignment(
                                guild.id, "general"
                            ),
                            "commander": self.db_manager.get_role_assignment(
                                guild.id, "commander"
                            ),
                            "sergeant": self.db_manager.get_role_assignment(
                                guild.id, "sergeant"
                            ),
                        }
                        ...
                        logger.error(
                            f"Error getting role assignments for guild {guild.id}: {e}",
                            exc_info=True,
                        )
            ...
            try:
                logger.debug(
                    f"Atomically deciding winner for {game_date} at {win_time}"
                )
                winner_result = self.game_logic.determine_and_save_winner(
                    game_date, win_time
                )
            ...
            if winner_result and winner_result.get("already_decided"):
                logger.info(
                    f"Winner for {game_date} already decided in a prior run; skipping announcement"
                )
                return
            ...
            if winner_result.get("catastrophic_event"):
                ...
                try:
                    ...
                    logger.error(
                        f"Error announcing catastrophic event: {e}", exc_info=True
                    )
                return

            logger.info("Winner saved to database successfully")
            try:
            ...
            logger.info(
                f"Winner determination complete for {game_date}: {winner_result.get('username', 'Unknown')}"
            )

        except Exception as e:
            logger.error(
                f"Critical error in _determine_daily_winner: {e}", exc_info=True
            )
...
The 1337 gods are not amused... try again tomorrow! 😤"""
            ...
            try:
                ...
                if Config.ANNOUNCEMENT_CHANNEL_ID:
                    ...
                    if (
                        ...
                        and announcement_channel.permissions_for(guild.me).send_messages
                    ):
                        await announcement_channel.send(message)
                        logger.info(
                            f"No winner announced in configured channel: {announcement_channel.name} (Guild: {guild.name})"
                        )
                        continue
                ...
                for channel in guild.text_channels:
                    if channel.permissions_for(guild.me).send_messages:
                        await channel.send(message)
                        logger.info(
                            f"No winner announced in fallback channel: {channel.name} (Guild: {guild.name})"
                        )
                        break

            except Exception as e:
                logger.error(
                    f"Error sending no winner announcement in guild {guild.id}: {e}"
                )
    ...
    async def _update_roles(self):
        ...
        try:
            ...
            if not any(
                [
                    Config.SERGEANT_ROLE_ID,
                    Config.COMMANDER_ROLE_ID,
                    Config.GENERAL_ROLE_ID,
                ]
            ):
            ...
            logger.info(
                f"Today's winner: {winner_today['username']} (ID: {winner_today['user_id']})"
            )
            ...
            if top_14_day:
                logger.info(
                    f"Top 14-day player: {top_14_day['username']} ({top_14_day['wins']} wins)"
                )
            if top_365_day:
                logger.info(
                    f"Top 365-day player: {top_365_day['username']} ({top_365_day['wins']} wins)"
                )

            for guild in self.bot.guilds:
                try:
                    await self._update_guild_roles(
                        guild, winner_today, top_14_day, top_365_day
                    )
                except Exception as e:
    ...
    async def _update_guild_roles(self, guild, winner_today, top_14_day, top_365_day):
        ...
        sergeant_role = (
            guild.get_role(Config.SERGEANT_ROLE_ID) if Config.SERGEANT_ROLE_ID else None
        )
        commander_role = (
            guild.get_role(Config.COMMANDER_ROLE_ID)
            if Config.COMMANDER_ROLE_ID
            else None
        )
        general_role = (
            guild.get_role(Config.GENERAL_ROLE_ID) if Config.GENERAL_ROLE_ID else None
        )
        ...
        new_assignments = self._determine_new_role_assignments(
            winner_today, current_roles, guild.id
        )
        logger.debug(f"New role assignments: {new_assignments}")

        # Update roles efficiently - only change what needs to be changed
        await self._update_role_assignments_efficiently(
            guild,
            current_roles,
            new_assignments,
            sergeant_role,
        ...
        for assignment in current_assignments:
            ...
            if member:
                ...
                if role and role in member.roles:
                    logger.info(
                        f"Removing {assignment['role_type']} role from {member.display_name}"
                    )
                    await member.remove_roles(role)
                else:
                    logger.warning(
                        f"Role {assignment['role_type']} not found or not assigned to {member.display_name}"
                    )
            else:
                logger.warning(
                    f"Member {assignment['user_id']} not found in guild {guild.name}"
                )
    ...
    def _determine_new_role_assignments(self, winner_today, current_roles, guild_id):
        ...
        return self.game_logic.determine_new_role_assignments(
            winner_today, current_roles, guild_id
        )

    async def _update_role_assignments_efficiently(
        self,
        guild,
        current_roles,
        new_assignments,
            ...
            if current_user_id != new_user_id:
                ...
                if current_user_id:
                    ...
                    if current_member and role in current_member.roles:
                        logger.info(
                            f"Removing {role_type} role from {current_member.display_name}"
                        )
                ...
                if new_user_id:
                    ...
                    if new_member:
                        logger.info(
                            f"Assigning {role_type} role to {new_member.display_name}"
                        )
                        await new_member.add_roles(role)
                        # Update database
                        success = self.db_manager.set_role_assignment(
                            guild.id, new_user_id, role_type, role.id
                        )
                        if not success:
                            logger.error(
                                f"Failed to update database for {role_type} assignment to {new_member.display_name}"
                            )
                    else:
                        logger.warning(
                            f"Member {new_user_id} not found in guild {guild.name}, skipping {role_type} assignment"
                        )
            else:
                logger.debug(f"No change needed for {role_type} role")

    async def _apply_new_role_assignments(
        self, guild, assignments, sergeant_role, commander_role, general_role
    ):
        ...
        for role_type, user_id in assignments.items():
            ...
            if not member:
                logger.warning(
                    f"Member {user_id} not found in guild {guild.name}, skipping {role_type} assignment"
                )
                continue
            ...
            # Update database
            success = self.db_manager.set_role_assignment(
                guild.id, user_id, role_type, role.id
            )
            if not success:
                logger.error(
                    f"Failed to update database for {role_type} assignment to {member.display_name}"
                )
    ...
    async def _announce_winner(self, winner_data, current_role_holders):
        ...
        try:
            ...
            await self._send_winner_announcement(
                winner_data, top_14_day, top_365_day, current_role_holders
            )
    ...
    async def _send_winner_announcement(
        self, winner_data, top_14_day, top_365_day, current_role_holders
    ):
        ...
        for guild in self.bot.guilds:
            try:
                logger.debug(
                    f"Processing announcement for guild: {guild.name} (ID: {guild.id})"
                )
                ...
                if guild_current_roles is None:
                    logger.warning(
                        f"Guild {guild.id} has None for current_role_holders - this may cause the NoneType error"
                    )
                    guild_current_roles = {}

                logger.debug(f"Creating winner message for guild {guild.id}")
                message = self.game_logic.create_winner_message(
                    winner_data, guild.id, guild_current_roles
                )
                logger.debug(
                    f"Created message for guild {guild.id}: {message[:100]}..."
                )
                ...
                if Config.ANNOUNCEMENT_CHANNEL_ID:
                    ...
                    if (
                        ...
                        await announcement_channel.send(message)
                        logger.info(
                            f"Winner announced in configured channel: {announcement_channel.name} (Guild: {guild.name})"
                        )
                        message_sent = True
                        continue
                ...
                for channel in guild.text_channels:
                    if channel.permissions_for(guild.me).send_messages:
                        await channel.send(message)
                        logger.info(
                            f"Winner announced in fallback channel: {channel.name} (Guild: {guild.name})"
                        )
                        message_sent = True
                        break
                ...
                logger.error(
                    f"Error sending winner announcement in guild {guild.id}: {e}",
                    exc_info=True,
                )
                logger.debug(
                    f"Guild {guild.id} details - Name: {guild.name}, current_role_holders entry: {current_role_holders.get(guild.id)}"
                )
