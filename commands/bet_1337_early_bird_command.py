...
class Bet1337EarlyBirdCommand(commands.Cog):
    ...
    @discord.app_commands.command(
        name="1337-early-bird",
        description="Place an early bird bet with a specific timestamp",
    )
    async def bet_1337_early_bird(
        self, interaction: discord.Interaction, timestamp: str
    ):
        ...
        try:
            ...
            if not bet_validation["valid"]:
                await interaction.response.send_message(
                    bet_validation["message"], ephemeral=True
                )
                return

            # Validate timestamp
            timestamp_validation = self.game_logic.validate_early_bird_timestamp(
                timestamp
            )
            if not timestamp_validation["valid"]:
                await interaction.response.send_message(
                    timestamp_validation["message"], ephemeral=True
                )
                return
