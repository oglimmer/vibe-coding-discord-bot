...
class GreetingsCommand(commands.Cog):
    ...
    @discord.app_commands.command(
        name="greetings", description="Show today's greeting activity"
    )
    async def greetings(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not greetings:
                embed.description = (
                    "No one has greeted today yet! Be the first to say good morning! 🌅"
                )
                ...
                sorted_greetings = sorted(
                    greetings, key=lambda x: x.reaction_count, reverse=True
                )
                lines = []
                for i, g in enumerate(sorted_greetings, 1):
                    ...
                    reaction_emoji = (
                        "🔥"
                        if g.reaction_count > 5
                        else "👍"
                        if g.reaction_count > 2
                        else "👋"
                    )
                    lines.append(
                        f"{i}. **{g.username}** - {g.reaction_count} reactions {reaction_emoji} (latest: {time_str})"
                    )

                embed.description = "\n".join(lines)
                embed.add_field(
                    name="Total Reactions Today",
                    value=str(total_reactions),
                    inline=True,
                )
                embed.add_field(
                    name="Unique Greeters", value=str(len(greetings)), inline=True
                )
                ...
                embed.add_field(
                    name="First Greeting",
                    value=f"{first.username} at {self.format_time(first.greeting_time)}",
                    inline=True,
                )
                if len(greetings) > 1:
                    embed.add_field(
                        name="Latest Greeting",
                        value=f"{last.username} at {self.format_time(last.greeting_time)}",
                        inline=True,
                    )
            ...
            logger.info(
                f"Greetings command used by {interaction.user} in {interaction.guild}"
            )
    ...
    @discord.app_commands.command(
        name="greetings-help",
        description="Learn about the greetings system and supported languages",
    )
