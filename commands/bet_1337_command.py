...
class Bet1337Command(commands.Cog):
    ...
    @discord.app_commands.command(
        name="1337", description="Place a regular bet for today's 1337 game"
    )
    async def bet_1337(self, interaction: discord.Interaction):
        """Handle regular bet placement"""
        try:
            logger.debug(
                f"User {interaction.user.display_name} (ID: {interaction.user.id}) attempting regular bet"
            )
            ...
            if not validation["valid"]:
                await interaction.response.send_message(
                    validation["message"], ephemeral=True
                )
                return
            ...
            logger.debug(
                f"Current time: {self.game_logic.format_time_with_ms(play_time)}"
            )
            ...
            if status == "saved":
                logger.info(
                    f"Regular bet saved successfully for {interaction.user.display_name} at {self.game_logic.format_time_with_ms(play_time)}"
                )
                ...
                if Config.GENERAL_ROLE_ID:
                    ...
                    if general_role and general_role in interaction.user.roles:
                        logger.debug("User is General, announcing bet")
                ...
                logger.info(
                    f"Bet from {interaction.user.display_name} lost race with winner determination"
                )
                await interaction.response.send_message(
                ...
                logger.error(
                    f"Failed to save regular bet for {interaction.user.display_name}"
                )
                await interaction.response.send_message(
                    "❌ **Error placing bet.** Please try again later.", ephemeral=True
                )
