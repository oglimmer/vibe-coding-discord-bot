...
class Rules1337Command(commands.Cog):
    ...
    @discord.app_commands.command(
        name="1337-rules",
        description="Display the complete 1337 game rules and mechanics",
    )
    async def rules_1337(self, interaction: discord.Interaction):
        """Display comprehensive game rules"""
        try:
            logger.debug(
                f"User {interaction.user.display_name} (ID: {interaction.user.id}) requested 1337 rules"
            )
    ...
    def _create_rules_embed(self) -> discord.Embed:
        ...
        embed.add_field(
            name="⏰ Game Schedule",
            value="• **Daily Game Time**: 13:37:00.000 (1:37 PM)\n"
            "• **Win Time Window**: Randomly generated between 13:37:00.000 and 13:38:00.000\n"
            "• **Frequency**: One game per day\n"
            "• **Winner Determination**: Automatic at exact win time with millisecond precision",
            inline=False,
        )
        ...
        embed.set_footer(
            text="The 1337 game combines precision timing, strategic planning, and community competition!"
        )
...
async def setup(bot, db_manager):
    ...
    await bot.add_cog(Rules1337Command(bot, db_manager))
