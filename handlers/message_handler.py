...
class MessageHandler:
    def __init__(self, db_manager: DatabaseManager):
        ...
        self.greeting_patterns = [
            rf"\b{re.escape(greeting)}\b" for greeting in all_greetings
        ]
        ...
        for pattern in self.greeting_patterns:
            ...
            if match:
                ...
                for i in range(len(words) - len(greeting_words) + 1):
                    if (
                        " ".join(words[i : i + len(greeting_words)]).lower()
                        == greeting_text.lower()
                    ):
    ...
    async def _handle_greeting(self, message):
        try:
            ...
            if greeting_id:
                ...
                logger.info(
                    f"Responded to greeting from {username} ({user_id}) with ID {greeting_id}"
                )
            else:
