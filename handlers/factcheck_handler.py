...
class FactCheckHandler:
    ...
    async def handle_factcheck_reaction(
        ...
        try:
            ...
            user_preference = self.db_manager.get_klugscheisser_preference(
                message_author.id
            )
            if not user_preference["opted_in"]:
                await self._send_opt_in_required_message(
                    reaction.message, user, message_author
                )
                return False
            ...
            async with reaction.message.channel.typing():
                ...
                logger.info(
                    f"Pre-checking factcheckability for message from {message_author.display_name}"
                )
                ...
                if is_factcheckable:
                    ...
                    logger.info("Message is factcheckable - performing full factcheck")
                    ...
                    if not factcheck_result:
                        logger.warning(
                            "No fact-check response received from OpenAI service"
                        )
                    ...
                    await reaction.message.reply(
                        formatted_response, mention_author=False
                    )

                    logger.info(
                        f"Successfully processed fact-check request from {user.display_name} "
                    ...
                    logger.info(
                        "Message is not factcheckable - sending private notification"
                    )
                    ...
                    logger.info(
                        f"Fact-check request from {user.display_name} completed - message not factcheckable"
                    )
                    return True  # Still counts as successfully processed

        except discord.HTTPException as e:
            logger.error(
                f"Failed to send fact-check response due to Discord API error: {e}"
            )
            return False
    ...
    async def _send_limit_exceeded_message(
        self, message: discord.Message, user: discord.User
    ):
    ...
    async def _send_opt_in_required_message(
        self,
    ...
    async def _send_message_too_short(
        self, message: discord.Message, user: discord.User
    ):
    ...
    async def _send_not_factcheckable_message(
        self, message: discord.Message, user: discord.User
    ):
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description="Diese Nachricht enthält keine überprüfbaren Fakten. "
                "Dein Limit wurde um 1 reduziert.",
                color=discord.Color.orange(),
            )
            embed.add_field(
                name="💡 Tipp",
            ...
            logger.info(
                f"Sent not-factcheckable ephemeral message for {user.display_name}"
            )
    ...
    def _format_factcheck_response(
        self,
...
*Hinweis: Dies ist eine automatisierte Bewertung und kann Fehler enthalten.*"""
        ...
        if len(formatted) > max_length:
            ...
            available_space = (
                max_length - len(formatted) + len(response) - 20
            )  # -20 for truncation indicator
