...
class KlugscheisserHandler:
    ...
    def __init__(self, db_manager):
        ...
        self.user_cooldowns: Dict[
            int, float
        ] = {}  # user_id -> last_klugscheisser_timestamp
    ...
    async def should_klugscheiss_message(self, message: discord.Message) -> bool:
        ...
        if Config.KLUGSCHEISSER_REQUIRE_OPTIN:
            ...
            if not user_preference["opted_in"]:
                logger.debug(
                    f"User {message.author.id} has not opted in to klugscheißer"
                )
                return False
        ...
        if should_trigger:
            logger.info(
                f"Klugscheißer triggered for message from {message.author.display_name} "
                f"(roll: {random_roll}/{Config.KLUGSCHEISSER_PROBABILITY}%)"
            )
        else:
            logger.debug(
                f"Klugscheißer not triggered (roll: {random_roll}/{Config.KLUGSCHEISSER_PROBABILITY}%)"
            )
    ...
    async def handle_klugscheisserei(self, message: discord.Message) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Checking if message from {message.author.display_name} is worth trolling"
            )
            should_troll = await self.openai_service.should_respond_with_klugscheiss(
                message.content
            )

            if not should_troll:
                logger.info(
                    f"Message from {message.author.display_name} not worth trolling - skipping"
                )
                return False  # Silent skip, no response

            # STEP 4: Generate troll response
            logger.info(
                f"Generating troll response for message from {message.author.display_name}"
            )
            ...
            logger.info(
                f"Successfully sent troll response for message from {message.author.display_name}"
            )
            return True

        except discord.HTTPException as e:
            logger.error(
                f"Failed to send klugscheißer response due to Discord API error: {e}"
            )
            return False
