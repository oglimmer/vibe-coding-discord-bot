...
class OpenAIService:
    ...
    def __init__(self):
        ...
        if Config.OPENAI_API_KEY:
            ...
            logger.warning(
                "OpenAI API key not configured. Factcheck feature will be disabled."
            )
    ...
    async def get_factcheck(
        self, message_content: str, user_name: str = None
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            response = await self.client.chat.completions.create(
                ...
                * 3,  # Allow longer responses for fact-checking
            ...
            if response.choices and response.choices[0].message:
                ...
                if content:
                    logger.info(
                        f"Successfully generated factcheck response ({len(content)} chars)"
                    )
...
"""
            ...
            logger.info(
                f"Factcheckability check for message '{message_content[:50]}...': {is_factcheckable} (AI response: {result})"
            )
...
"""
            ...
            logger.info(
                f"Klugscheiss-check for message '{message_content[:50]}...': {should_troll} (AI response: {result})"
            )
            return should_troll
    ...
    async def generate_klugscheiss_response(
        self, message_content: str, user_name: str = None, db_manager=None
    ) -> Optional[str]:
        """
...
"""
            ...
            if response.choices and response.choices[0].message:
                ...
                if content:
                    logger.info(
                        f"Successfully generated klugscheiss response ({len(content)} chars)"
                    )
                    # Save to cache
                    if db_manager:
                        db_manager.save_ai_response_cache(
                            message_content, "klugscheiss", content.strip()
                        )
                    return content.strip()
    ...
    async def get_reaction_factcheck(
        self, message_content: str, user_name: str = None
    ) -> Optional[dict]:
        """
...
{"score": [Ganzzahl 0-100], "explanation": "[Kurze deutsche Erklärung der Bewertung]"}""",
                ...
                if content:
                    ...
                    try:
                        ...
                        if "score" in result and "explanation" in result:
                            ...
                            if 0 <= score <= 100:
                                logger.info(
                                    f"Successfully generated reaction factcheck with score {score}%"
                                )
                                return {
                                    "score": score,
                                ...
                                logger.warning(
                                    f"Invalid score received: {score}% (must be 0-100)"
                                )
                        else:
                            logger.warning(
                                "Missing required fields in factcheck response"
                            )
    ...
    def _create_reaction_factcheck_prompt(
        self, message_content: str, user_name: str = None
    ) -> str:
    ...
    def _extract_score_from_text(self, text: str) -> Optional[dict]:
        ...
        if score_match:
            try:
                ...
                if 0 <= score <= 100:
                    return {"score": score, "response": text.strip()}
            except ValueError:
                pass
